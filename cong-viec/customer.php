    ...
    switch ($_POST['action']) {
        case 'update_info':
            ...
            if ($relMe)
                $relArr[] = "Mẹ: " . $relMe;
            if ($relVoChong)
                $relArr[] = "Vợ/chồng: " . $relVoChong;
            if ($relAnhChiEm)
                $relArr[] = "Anh/chị/em: " . $relAnhChiEm;
            if ($relDongNghiep)
                $relArr[] = "Đồng nghiệp: " . $relDongNghiep;
            if ($relKhac)
                $relArr[] = "Khác: " . $relKhac;
        ...
        case 'upload_file':
            if (isset($_FILES['file']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
                ...
                $categoryNames = [
                    'customer_photo' => 'Ảnh khách',
                    'image' => 'Hình ảnh',
                    'video' => 'Video',
                    'document' => 'Tài liệu',
                    'tin_dung_1' => 'Tín dụng 1',
                    'tin_dung_2' => 'Tín dụng 2',
                    'cnc' => 'CNC',
                    'don_thu' => 'Đơn Thư',
                    'truc_fb' => 'Trực FB',
                    'ke_toan' => 'Kế Toán',
                ...
                try {
                    ...
                    // Tạo subfolder theo category trong folder khách
                    $catFolderName = $categoryNames[$fileCategory] ?? 'Tài liệu';
                    $catFolderId = $drive->findOrCreateFolder($catFolderName, $custFolderId);

                    // Đặt tên file theo phân loại
                    $fileLabel = $_POST['file_label'] ?? '';
                    $customLabel = trim($_POST['file_label_custom'] ?? '');
                    $ext = strtolower(pathinfo($_FILES['file']['name'], PATHINFO_EXTENSION));
                    $dateStr = date('dmy');
                    
                    $labelMap = [
                        'hopdong' => 'hopdong',
                        'video_hopdong' => 'video_hopdong',
                        'anhmat' => 'anhmat',
                        'donthu' => 'donthu',
                    ];
                    
                    if ($fileLabel === 'khac' && $customLabel) {
                        // Chuyển tên tùy chọn thành không dấu
                        $slug = preg_replace('/[^a-z0-9]+/', '_', strtolower($customLabel));
                        $slug = trim($slug, '_');
                        $uploadFileName = $slug . '_' . $dateStr . '.' . $ext;
                    } elseif (isset($labelMap[$fileLabel])) {
                        $uploadFileName = $labelMap[$fileLabel] . '_' . $dateStr . '.' . $ext;
                    } else {
                        $uploadFileName = $_FILES['file']['name'];
                    }

                    // Upload file vào subfolder category
...
// File đính kèm — thêm cột category nếu chưa có
try {
    $pdo->exec("ALTER TABLE cv_customer_files ADD COLUMN category VARCHAR(30) DEFAULT 'document'");
} catch (Exception $e) {
}
...
// Đếm file theo category
$fileCounts = ['customer_photo' => 0, 'image' => 0, 'video' => 0, 'document' => 0, 'tin_dung_1' => 0, 'tin_dung_2' => 0, 'cnc' => 0, 'don_thu' => 0, 'truc_fb' => 0, 'ke_toan' => 0, 'hop_ban' => 0];
foreach ($customerFiles as $cf) {
    $cat = $cf['category'] ?? 'document';
    if (isset($fileCounts[$cat]))
        $fileCounts[$cat]++;
}
...
foreach ($workLogs as $wl) {
    $parts = [];
    if ($wl['room_name'])
        $parts[] = 'Phòng làm việc: ' . $wl['room_name'];
    if ($wl['action_type'])
        $parts[] = 'Việc đã làm: ' . $wl['action_type'];
    if ($wl['result_type'])
        $parts[] = 'Kết quả: ' . $wl['result_type'];
    if ($wl['amount'])
        $parts[] = 'Tiền lãi: ' . number_format($wl['amount'], 0, ',', '.') . 'đ';
    if (!empty($wl['amount_principal']))
        $parts[] = 'Tiền gốc: ' . number_format($wl['amount_principal'], 0, ',', '.') . 'đ';
    if ($wl['promise_date'])
        $parts[] = 'Ngày hẹn: ' . date('d/m/Y', strtotime($wl['promise_date']));
    if ($wl['work_done'] && $wl['work_done'] !== ($wl['action_type'] . ' → ' . $wl['result_type']))
        $parts[] = 'Ghi chú: ' . $wl['work_done'];
    $timeline[] = [
...
if ($customer['planned_next_room_id']) {
    ...
    if ($plannedRoom)
        $plannedRoomName = $plannedRoom['icon'] . ' ' . $plannedRoom['name'];
}
        ...
        <div style="display:flex;align-items:center;gap:16px;">
            ...
            <div>
                ...
                <div style="display:flex;align-items:center;gap:12px;margin-top:6px;flex-wrap:wrap;">
                    <button class="btn btn-ghost btn-sm" onclick="openModal('change-room-modal')">🔄 Chuyển qua
                        Phòng</button>
                    <button class="btn btn-ghost btn-sm" onclick="openModal('worklog-modal')">📝 Lưu Nhật Ký</button>
                    <a href="../contracts.php" class="btn btn-ghost btn-sm"
                        style="text-decoration:none;color:var(--accent-blue);" target="_blank">💰 Tài chính</a>
                    <span style="font-size:13px;">
                        <span style="margin-right:4px"><?= $customer['room_icon'] ?></span>
                        <?= sanitize($customer['room_name']) ?>
                        <?php if ($customer['sla_days'] > 0): ?>
                            <span style="color:var(--text-muted);font-size:11px;">(Hạn: <?= $customer['sla_days'] ?>
                                ngày)</span>
                        <?php endif; ?>
                    </span>
                    ...
                    <span class="customer-status"
                        style="background:<?= $statusColor ?>20; color:<?= $statusColor ?>; font-size:13px;">
                        <span class="status-dot" style="background:<?= $statusColor ?>"></span>
                        <?= $statusText ?>
                    </span>
                    <?php if ($plannedRoomName): ?>
                        <span class="tag" style="--tag-bg:rgba(35,131,226,0.15);--tag-color:#2383e2;">→
                            <?= sanitize($plannedRoomName) ?></span>
                    <?php endif; ?>
                </div>
            </div>
        </div>
...
<div class="page-body">
    ...
    if ($hasExtraInfo): ?>
        <section style="margin-bottom:28px;">
            <div
                style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:12px;background:var(--bg-card);border:1px solid var(--border-color);border-radius:var(--radius-lg);padding:16px;">
                <?php if ($customer['phone']): ?>
                    <div><span style="color:var(--text-muted);font-size:12px;">📱 SĐT</span><br><strong
                            style="font-size:14px;"><?= sanitize($customer['phone']) ?></strong></div>
                <?php endif; ?>
                <?php if ($customer['gender']): ?>
                    <div><span style="color:var(--text-muted);font-size:12px;">⚧ Giới tính</span><br><strong
                            style="font-size:14px;"><?= sanitize($customer['gender']) ?></strong></div>
                <?php endif; ?>
                <?php if ($customer['date_of_birth']): ?>
                    <div><span style="color:var(--text-muted);font-size:12px;">🎂 Ngày sinh</span><br><strong
                            style="font-size:14px;"><?= date('d/m/Y', strtotime($customer['date_of_birth'])) ?></strong></div>
                <?php endif; ?>
                <?php if ($customer['cccd']): ?>
                    <div><span style="color:var(--text-muted);font-size:12px;">🪪 CCCD</span><br><strong
                            style="font-size:14px;"><?= sanitize($customer['cccd']) ?></strong></div>
                <?php endif; ?>
                <?php if ($customer['company_tag']): ?>
                    <div><span style="color:var(--text-muted);font-size:12px;">🏢 Công ty</span><br><strong
                            style="font-size:14px;"><?= sanitize($customer['company_tag']) ?></strong></div>
                <?php endif; ?>
                <?php if ($customer['address']): ?>
                    <div><span style="color:var(--text-muted);font-size:12px;">📍 Địa chỉ</span><br><span
                            style="font-size:14px;"><?= sanitize($customer['address']) ?></span></div>
                <?php endif; ?>
                <?php if ($customer['facebook_link']): ?>
                    <div><span style="color:var(--text-muted);font-size:12px;">📘 Facebook</span><br><a
                            href="<?= sanitize($customer['facebook_link']) ?>" target="_blank" style="font-size:14px;">Xem trang
                            →</a></div>
                <?php endif; ?>
                <?php if ($customer['relatives_info']): ?>
                    <div style="grid-column:1/-1;"><span style="color:var(--text-muted);font-size:12px;">👨‍👩‍👧 Người
                            thân</span><br><span
                            style="font-size:14px;white-space:pre-wrap;"><?= sanitize($customer['relatives_info']) ?></span>
                    </div>
                <?php endif; ?>
            </div>
        </section>
    ...
    <?php if ($customer['loan_amount']): ?>
        <section style="margin-bottom:20px;">
            <div
                style="background:var(--bg-card);border:1px solid var(--border-color);border-radius:var(--radius-lg);padding:16px;border-left:3px solid var(--accent-blue);">
                <div style="margin-bottom:8px;">
                    <span style="color:var(--accent-blue);font-size:12px;font-weight:600;">💰 THÔNG TIN TÀI CHÍNH (chỉ
                        xem)</span>
                    <?php if ($customer['loan_code']): ?>
                        <span style="float:right;font-size:12px;color:var(--text-muted);">Mã HĐ:
                            <strong><?= sanitize($customer['loan_code']) ?></strong></span>
                    <?php endif; ?>
                </div>
                <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:12px;">
                    ...
                    <?php if ($customer['next_payment_date']): ?>
                        <div>
                            ...
                            <strong
                                style="font-size:15px;color:<?= strtotime($customer['next_payment_date']) <= time() ? 'var(--accent-red)' : 'var(--accent-green)' ?>;">
                    ...
                    <div>
                        ...
                        ?>
                        <strong
                            style="font-size:13px;"><?= $loanStatusLabels[$customer['loan_status']] ?? $customer['loan_status'] ?></strong>
                    </div>
                </div>
            </div>
        </section>
    ...
    <section style="margin-bottom:20px;">
        <div
            style="background:var(--bg-card);border:1px solid var(--border-color);border-radius:var(--radius-lg);padding:14px 16px;border-left:3px solid #f59e0b;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
                <span style="color:#f59e0b;font-size:12px;font-weight:600;">📌 NỘI DUNG CẦN LƯU Ý</span>
                <button class="btn btn-ghost btn-sm" onclick="togglePinnedEdit()"
                    style="color:var(--accent-blue);font-size:12px;">✏️ Sửa</button>
            </div>
            ...
            <form method="POST" id="pinned-edit" style="display:none;margin-top:8px;">
                ...
                <textarea name="pinned_note" class="form-textarea"
                    style="margin-bottom:8px;min-height:80px;"><?= sanitize($customer['pinned_note'] ?? '') ?></textarea>
    ...
    <section style="margin-bottom:20px;">
        <div
            style="background:var(--bg-card);border:1px solid var(--border-color);border-radius:var(--radius-lg);padding:14px 16px;border-left:3px solid #8b5cf6;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                <span style="color:#8b5cf6;font-size:12px;font-weight:600;">🤖 AI ĐÁNH GIÁ KHÁCH HÀNG</span>
                <div style="display:flex;gap:6px;">
                    <button class="btn btn-ghost btn-sm" onclick="toggleAiPrompt()"
                        style="color:var(--text-muted);font-size:11px;" title="Tùy chỉnh prompt">⚙️</button>
                    <button class="btn btn-sm" id="ai-analyze-btn" onclick="runAiAnalysis()"
                        style="background:#8b5cf6;color:#fff;border:none;border-radius:6px;padding:4px 12px;font-size:12px;cursor:pointer;">
                        ✨ Phân tích
                    </button>
                </div>
            </div>
            ...
            <div id="ai-prompt-box" style="display:none;margin-bottom:10px;">
                <textarea id="ai-custom-prompt" class="form-textarea" style="min-height:80px;font-size:12px;"
                    placeholder="Để trống = dùng prompt mặc định. Hoặc nhập prompt riêng..."></textarea>
                <div style="font-size:11px;color:var(--text-muted);margin-top:4px;">💡 Prompt mặc định đánh giá: Mức độ
                    hợp tác, Khả năng thu hồi, Rủi ro, Đề xuất hành động</div>
            </div>
            <!-- Loading -->
            <div id="ai-loading" style="display:none;text-align:center;padding:20px;">
                <div
                    style="display:inline-block;width:24px;height:24px;border:3px solid #8b5cf620;border-top-color:#8b5cf6;border-radius:50%;animation:spin 0.8s linear infinite;">
                </div>
            ...
            <div id="ai-result" style="display:none;">
                <div id="ai-result-content"
                    style="font-size:14px;line-height:1.8;color:var(--text-secondary);white-space:pre-wrap;"></div>
                <div id="ai-result-meta"
                    style="margin-top:10px;font-size:11px;color:var(--text-muted);border-top:1px solid var(--border-color);padding-top:8px;">
                </div>
            </div>
            <!-- Lỗi -->
            <div id="ai-error"
                style="display:none;padding:12px;background:rgba(239,68,68,0.1);border-radius:8px;color:#ef4444;font-size:13px;">
            </div>
            <!-- Chưa phân tích -->
            <div id="ai-empty" style="color:var(--text-muted);font-size:13px;font-style:italic;">Bấm "Phân tích" để AI
    ...
    <section style="margin-bottom:28px;">
        <form method="POST" id="desc-form">
            ...
            <div
                style="background:var(--bg-card);border:1px solid var(--border-color);border-radius:var(--radius-lg);padding:16px;">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                    <span style="color:var(--text-muted);font-size:12px;font-weight:600;">📋 THÔNG TIN KHÁCH HÀNG</span>
                    <div>
                        <button type="button" class="btn btn-ghost btn-sm" id="desc-edit-btn" onclick="enableDescEdit()"
                            style="color:var(--accent-blue);">✏️ Sửa</button>
                        <button type="submit" class="btn btn-primary btn-sm" id="desc-save-btn" style="display:none;">💾
                            Lưu</button>
                        <button type="button" class="btn btn-ghost btn-sm" id="desc-cancel-btn"
                            style="display:none;color:var(--text-muted);" onclick="cancelDescEdit()">Hủy</button>
                    </div>
                </div>
                <!-- Hiển thị chế độ xem -->
                <div id="desc-display"
    ...
    <?php if (!empty($customer['tc_info'])): ?>
        ...
        <section style="margin-bottom:28px;">
            <div
                style="background:var(--bg-card);border:1px solid var(--border-color);border-radius:var(--radius-lg);padding:16px;border-left:3px solid var(--accent-orange);">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                    <span style="color:var(--accent-orange);font-size:12px;font-weight:600;">🔗 THÔNG TIN TỪ TÀI
                        CHÍNH</span>
    ...
    <div style="display:flex;gap:4px;margin-bottom:20px;border-bottom:1px solid var(--border-color);padding-bottom:0;">
        <a href="?id=<?= $customerId ?>&tab=comments" class="btn btn-ghost btn-sm"
            style="border-radius:var(--radius-md) var(--radius-md) 0 0;padding:8px 16px;<?= $tab === 'comments' ? 'background:var(--bg-card);color:var(--text-primary);border:1px solid var(--border-color);border-bottom:1px solid var(--bg-primary);margin-bottom:-1px;' : '' ?>">
            💬 Bình luận (<?= count($comments) ?>)
        </a>
        <a href="?id=<?= $customerId ?>&tab=worklog" class="btn btn-ghost btn-sm"
            style="border-radius:var(--radius-md) var(--radius-md) 0 0;padding:8px 16px;<?= $tab === 'worklog' ? 'background:var(--bg-card);color:var(--text-primary);border:1px solid var(--border-color);border-bottom:1px solid var(--bg-primary);margin-bottom:-1px;' : '' ?>">
            📋 Nội dung công việc (<?= count($workLogs) ?>)
        </a>
        <a href="?id=<?= $customerId ?>&tab=transfers" class="btn btn-ghost btn-sm"
            style="border-radius:var(--radius-md) var(--radius-md) 0 0;padding:8px 16px;<?= $tab === 'transfers' ? 'background:var(--bg-card);color:var(--text-primary);border:1px solid var(--border-color);border-bottom:1px solid var(--bg-primary);margin-bottom:-1px;' : '' ?>">
            📜 Chuyển phòng (<?= count($transferLogs) ?>)
        </a>
        <a href="?id=<?= $customerId ?>&tab=files" class="btn btn-ghost btn-sm"
            style="border-radius:var(--radius-md) var(--radius-md) 0 0;padding:8px 16px;<?= $tab === 'files' ? 'background:var(--bg-card);color:var(--text-primary);border:1px solid var(--border-color);border-bottom:1px solid var(--bg-primary);margin-bottom:-1px;' : '' ?>">
            📎 File (<?= count($customerFiles) ?>)
        </a>
        <a href="?id=<?= $customerId ?>&tab=timeline" class="btn btn-ghost btn-sm"
            style="border-radius:var(--radius-md) var(--radius-md) 0 0;padding:8px 16px;<?= $tab === 'timeline' ? 'background:var(--bg-card);color:var(--text-primary);border:1px solid var(--border-color);border-bottom:1px solid var(--bg-primary);margin-bottom:-1px;' : '' ?>">
    ...
    <?php if ($tab === 'comments'): ?>
        <section>
                ...
                <div style="display:flex;gap:12px;margin-bottom:16px;">
                    ...
                    <div style="flex:1;">
                        <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
                            ...
                            <span
                                style="font-size:12px;color:var(--text-muted);"><?= date('d/m/Y H:i', strtotime($cm['created_at'])) ?></span>
                        </div>
            ...
            <form method="POST" style="display:flex;gap:8px;align-items:flex-start;">
                ...
                <div class="user-avatar" style="flex-shrink:0;margin-top:2px;">
                    <?= mb_substr($user['fullname'], 0, 1, 'UTF-8') ?>
                </div>
                <textarea name="content" class="form-textarea" placeholder="Thêm bình luận..."
                    style="min-height:60px;flex:1;" required></textarea>
                        ...
                        <?php foreach ($workLogs as $wl): ?>
                            <tr>
                                ...
                                <td>
                                    <?php if ($wl['amount']): ?>
                                        <strong
                                            style="color:var(--accent-green)"><?= number_format($wl['amount'], 0, ',', '.') ?>₫</strong>
                                    <?php else: ?>
                                        <span style="color:var(--text-muted)">—</span>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <?php if (!empty($wl['amount_principal'])): ?>
                                        <strong
                                            style="color:var(--accent-blue)"><?= number_format($wl['amount_principal'], 0, ',', '.') ?>₫</strong>
                                    <?php else: ?>
                                        <span style="color:var(--text-muted)">—</span>
                                    <?php endif; ?>
                                </td>
    ...
    <?php if ($tab === 'transfers'): ?>
        <section>
            <div style="display:flex;align-items:center;justify-content:flex-end;gap:8px;margin-bottom:16px;">
                <a href="?id=<?= $customerId ?>&tab=transfers&export=xlsx" class="btn btn-secondary btn-sm">📥 Xuất
                    Excel</a>
            </div>
    ...
    <?php if ($tab === 'files'): ?>
        <?php
        $activeFolder = $_GET['folder'] ?? '';
        $fileCategories = [
            'customer_photo' => ['icon' => '👤', 'name' => 'Ảnh khách', 'desc' => 'CCCD, ảnh mặt, giấy tờ KH', 'color' => '#e91e63'],
            'image'          => ['icon' => '🖼️', 'name' => 'Hình ảnh', 'desc' => 'Ảnh chụp, screenshot...', 'color' => '#4caf50'],
            'video'          => ['icon' => '🎬', 'name' => 'Video', 'desc' => 'Video ghi hình, bằng chứng', 'color' => '#ff9800'],
            'document'       => ['icon' => '📄', 'name' => 'Tài liệu', 'desc' => 'PDF, Word, Excel...', 'color' => '#2196f3'],
            'tin_dung_1'     => ['icon' => '🏦', 'name' => 'Tín dụng 1', 'desc' => 'Hồ sơ phòng Tín dụng 1', 'color' => '#00bcd4'],
            'tin_dung_2'     => ['icon' => '🏛️', 'name' => 'Tín dụng 2', 'desc' => 'Hồ sơ phòng Tín dụng 2', 'color' => '#009688'],
            'cnc'            => ['icon' => '⚙️', 'name' => 'CNC', 'desc' => 'Hồ sơ CNC', 'color' => '#607d8b'],
            'don_thu'        => ['icon' => '✉️', 'name' => 'Đơn Thư', 'desc' => 'Đơn thư, văn bản gửi', 'color' => '#795548'],
            'truc_fb'        => ['icon' => '💬', 'name' => 'Trực FB', 'desc' => 'Hồ sơ trực Facebook', 'color' => '#1877f2'],
            'ke_toan'        => ['icon' => '📊', 'name' => 'Kế Toán', 'desc' => 'Chứng từ, sổ sách kế toán', 'color' => '#4caf50'],
            'hop_ban'        => ['icon' => '🤝', 'name' => 'Họp bàn', 'desc' => 'Biên bản họp, ghi chú', 'color' => '#ff5722'],
        ];
        ?>
        <section>
            <?php if (empty($activeFolder)): ?>
                <!-- FOLDER GRID -->
                <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:12px;">
                    <?php foreach ($fileCategories as $catKey => $catInfo): ?>
                        <a href="?id=<?= $customerId ?>&tab=files&folder=<?= $catKey ?>"
                            style="background:var(--bg-card);border:1px solid var(--border-color);border-radius:var(--radius-lg);padding:20px;text-decoration:none;transition:all 0.2s;display:block;"
                            onmouseover="this.style.borderColor='<?= $catInfo['color'] ?>';this.style.transform='translateY(-2px)';this.style.boxShadow='0 4px 12px rgba(0,0,0,0.15)'"
                            onmouseout="this.style.borderColor='var(--border-color)';this.style.transform='';this.style.boxShadow=''">
                            <div style="font-size:36px;margin-bottom:8px;"><?= $catInfo['icon'] ?></div>
                            <div style="font-weight:600;color:var(--text-primary);font-size:14px;"><?= $catInfo['name'] ?></div>
                            <div style="color:var(--text-muted);font-size:12px;margin-top:2px;"><?= $catInfo['desc'] ?></div>
                            <div
                                style="margin-top:10px;display:inline-block;background:<?= $catInfo['color'] ?>22;color:<?= $catInfo['color'] ?>;padding:2px 10px;border-radius:12px;font-size:12px;font-weight:600;">
                                <?= $fileCounts[$catKey] ?? 0 ?> file
                            </div>
                        </a>
                    <?php endforeach; ?>
                </div>
            ...
            <?php else: ?>
                <!-- FOLDER CONTENT -->
                <?php
                $folderInfo = $fileCategories[$activeFolder] ?? $fileCategories['document'];
                $folderFiles = array_filter($customerFiles, fn($f) => ($f['category'] ?? 'document') === $activeFolder);
                ?>
                <div style="margin-bottom:16px;">
                    <a href="?id=<?= $customerId ?>&tab=files"
                        style="color:var(--text-muted);text-decoration:none;font-size:13px;">
                        ← Quay lại thư mục
                    </a>
                </div>
                <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px;">
                    <span style="font-size:28px;"><?= $folderInfo['icon'] ?></span>
                    <div>
                        <h3 style="margin:0;font-size:16px;"><?= $folderInfo['name'] ?></h3>
                        <div style="color:var(--text-muted);font-size:12px;"><?= count($folderFiles) ?> file</div>
                    </div>
                </div>
                ...
                <form method="POST" enctype="multipart/form-data" style="margin-bottom:20px;">
                    ...
                    <div style="background:var(--bg-card);border:2px dashed var(--border-color);border-radius:var(--radius-lg);padding:24px;text-align:center;cursor:pointer;transition:all 0.2s;"
                        id="file-drop-zone" onclick="document.getElementById('file-input').click()"
                        ondragover="event.preventDefault();this.style.borderColor='<?= $folderInfo['color'] ?>';this.style.background='rgba(0,0,0,0.05)'"
                        ...
                        <div style="font-size:32px;margin-bottom:8px;"><?= $folderInfo['icon'] ?></div>
                        <div style="color:var(--text-secondary);font-size:14px;">Kéo thả file vào đây hoặc <span
                                style="color:<?= $folderInfo['color'] ?>;text-decoration:underline;">chọn file</span></div>
                        <div style="color:var(--text-muted);font-size:12px;margin-top:4px;">Upload vào thư mục:
                            <?= $folderInfo['name'] ?>
                        </div>
                        <div id="file-name-display"
                            style="color:<?= $folderInfo['color'] ?>;font-size:13px;margin-top:8px;font-weight:600;"></div>
                    </div>
                    <input type="file" id="file-input" name="file" style="display:none"
                        onchange="document.getElementById('file-name-display').textContent=this.files[0]?.name||'';document.getElementById('file-upload-options').style.display=this.files[0]?'':'none';document.getElementById('file-submit-btn').style.display=this.files[0]?'':'none'">
                    
                    <!-- File label selection -->
                    <div id="file-upload-options" style="display:none;margin-top:12px;background:var(--bg-card);border:1px solid var(--border-color);border-radius:var(--radius-lg);padding:16px;">
                        <label style="font-size:13px;font-weight:600;color:var(--text-secondary);display:block;margin-bottom:8px;">📋 Phân loại tài liệu:</label>
                        <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:8px;">
                            <label style="display:flex;align-items:center;gap:4px;padding:6px 12px;background:var(--bg-card);border:1px solid var(--border-color);border-radius:8px;cursor:pointer;font-size:13px;transition:all 0.2s;">
                                <input type="radio" name="file_label" value="hopdong" onchange="updateFileName(this)" style="accent-color:<?= $folderInfo['color'] ?>"> 📝 Hợp đồng
                            </label>
                            <label style="display:flex;align-items:center;gap:4px;padding:6px 12px;background:var(--bg-card);border:1px solid var(--border-color);border-radius:8px;cursor:pointer;font-size:13px;transition:all 0.2s;">
                                <input type="radio" name="file_label" value="video_hopdong" onchange="updateFileName(this)" style="accent-color:<?= $folderInfo['color'] ?>"> 🎬 Video làm HĐ
                            </label>
                            <label style="display:flex;align-items:center;gap:4px;padding:6px 12px;background:var(--bg-card);border:1px solid var(--border-color);border-radius:8px;cursor:pointer;font-size:13px;transition:all 0.2s;">
                                <input type="radio" name="file_label" value="anhmat" onchange="updateFileName(this)" style="accent-color:<?= $folderInfo['color'] ?>"> 👤 Ảnh mặt khách
                            </label>
                            <label style="display:flex;align-items:center;gap:4px;padding:6px 12px;background:var(--bg-card);border:1px solid var(--border-color);border-radius:8px;cursor:pointer;font-size:13px;transition:all 0.2s;">
                                <input type="radio" name="file_label" value="donthu" onchange="updateFileName(this)" style="accent-color:<?= $folderInfo['color'] ?>"> ✉️ Đơn thư
                            </label>
                            <label style="display:flex;align-items:center;gap:4px;padding:6px 12px;background:var(--bg-card);border:1px solid var(--border-color);border-radius:8px;cursor:pointer;font-size:13px;transition:all 0.2s;">
                                <input type="radio" name="file_label" value="khac" onchange="updateFileName(this);document.getElementById('custom-label-input').style.display=''" style="accent-color:<?= $folderInfo['color'] ?>"> ✏️ Khác
                            </label>
                        </div>
                        <input type="text" id="custom-label-input" name="file_label_custom" placeholder="Nhập tên tùy chọn..." style="display:none;width:100%;padding:8px 12px;border:1px solid var(--border-color);border-radius:8px;background:var(--bg-card);color:var(--text-primary);font-size:13px;margin-bottom:8px;" onkeyup="updateFileName(this)">
                        <div style="font-size:12px;color:var(--text-muted);">📎 Tên file: <strong id="file-preview-name" style="color:<?= $folderInfo['color'] ?>">—</strong></div>
                    </div>

                    <div style="text-align:right;margin-top:8px;">
                        <button type="submit" class="btn btn-primary btn-sm" id="file-submit-btn" style="display:none;">⬆️
                            Upload lên Drive</button>
                    </div>
                </form>
                ...
                function updateFileName(el) {
                    ...
                    var origName = fileInput.files[0].name;
                    ...
                    var labels = {hopdong:'hopdong', video_hopdong:'video_hopdong', anhmat:'anhmat', donthu:'donthu'};
                    var selected = document.querySelector('input[name=file_label]:checked');
                    if (!selected) { document.getElementById('file-preview-name').textContent = origName; return; }
                    
                    var val = selected.value;
                    var customInput = document.getElementById('custom-label-input');
                    
                    if (val === 'khac') {
                        customInput.style.display = '';
                        var custom = customInput.value.trim();
                        if (custom) {
                            var slug = custom.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_|_$/g, '');
                            document.getElementById('file-preview-name').textContent = slug + '_' + dateStr + '.' + ext;
                        } else {
                            document.getElementById('file-preview-name').textContent = 'khac_' + dateStr + '.' + ext;
                        }
                    } else {
                        customInput.style.display = 'none';
                        document.getElementById('file-preview-name').textContent = (labels[val]||val) + '_' + dateStr + '.' + ext;
                    }
                }
                </script>
                ...
                <?php if (empty($folderFiles)): ?>
                    <div class="empty-state" style="padding:24px;">
                        <div class="empty-state-text">Chưa có file trong thư mục này</div>
                    </div>
                <?php else: ?>
                    <table class="data-table">
                        <thead>
                        ...
                        <tbody>
                            <?php foreach ($folderFiles as $cf): ?>
                                <tr>
                                    <td>
                                        <a href="<?= sanitize($cf['drive_link']) ?>" target="_blank"
                                            style="color:var(--accent-blue);text-decoration:none;font-weight:500;">
                                            <?php
                                            $ext = strtolower(pathinfo($cf['file_name'], PATHINFO_EXTENSION));
                                            $icons = ['pdf' => '📕', 'doc' => '📘', 'docx' => '📘', 'xls' => '📗', 'xlsx' => '📗', 'jpg' => '🖼️', 'jpeg' => '🖼️', 'png' => '🖼️', 'gif' => '🖼️', 'mp4' => '🎬', 'mov' => '🎬', 'avi' => '🎬'];
                                            echo ($icons[$ext] ?? '📄') . ' ';
                                            ?>
                                    ...
                                    <td style="white-space:nowrap;font-size:13px;"><?= date('d/m/Y', strtotime($cf['created_at'])) ?>
                                    </td>
                                    <td style="text-align:right;">
                                        <a href="<?= sanitize($cf['drive_link']) ?>" target="_blank" class="btn btn-ghost btn-sm"
                                            title="Xem">👁️</a>
                                        <form method="POST" style="display:inline" onsubmit="return confirm('Xóa file này?')">
                                            ...
                                            <button type="submit" class="btn btn-ghost btn-sm" style="color:var(--status-danger);"
                                                title="Xóa">🗑️</button>
                                        </form>
                                    </td>
                                </tr>
                    ...
                    <table class="data-table">
                        ...
                        <tbody>
                            ...
                            foreach ($timeline as $item):
                                ...
                                ?>
                                <tr>
                                    <td style="white-space:nowrap;font-size:13px;"><?= date('d/m/Y', strtotime($item['date'])) ?>
                                    </td>
                                    <td style="font-size:13px;"><?= sanitize($customer['name']) ?></td>
                                    <td>
                                        <span
                                            style="font-size:12px;padding:2px 8px;border-radius:var(--radius-sm);background:<?= $tc['bg'] ?>;color:<?= $tc['color'] ?>;">
                                            <?= $item['type'] ?>
                                        </span>
                                    </td>
                                    <td style="font-size:13px;line-height:1.5;max-width:400px;"><?= sanitize($item['summary']) ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
        ...
        <form method="POST">
            ...
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <input type="text" name="name" class="form-input" value="<?= sanitize($customer['name']) ?>"
                        required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="address" class="form-input"
                        value="<?= sanitize($customer['address'] ?? '') ?>">
                </div>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                    <div class="form-group">
                        <label class="form-label">📘 Facebook Link</label>
                        <input type="url" name="facebook_link" class="form-input"
                            value="<?= sanitize($customer['facebook_link'] ?? '') ?>">
                    </div>
                    <div class="form-group">
                        <label class="form-label">🏢 Thuộc công ty</label>
                        <input type="text" name="company_tag" class="form-input"
                            value="<?= sanitize($customer['company_tag'] ?? '') ?>">
                    </div>
                </div>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                    <div class="form-group">
                        ...
                        <input type="text" name="workplace" class="form-input"
                            value="<?= sanitize($customer['workplace'] ?? '') ?>">
                    </div>
                    <div class="form-group">
                        <label class="form-label">🏠 HKTT</label>
                        <input type="text" name="hktt" class="form-input"
                            value="<?= sanitize($customer['hktt'] ?? '') ?>">
                    </div>
                </div>
                <div class="form-group">
                    <?php
                    ...
                    <div
                        style="display:grid;grid-template-columns:1fr 1fr;gap:12px;background:var(--bg-primary);padding:12px;border-radius:var(--radius-md);border:1px solid var(--border-color);">
                        <div>
                            <label
                                style="font-size:12px;color:var(--text-muted);display:block;margin-bottom:4px;">Bố</label>
                            <input type="text" name="rel_bo" class="form-input" style="font-size:13px;padding:6px 10px;"
                                value="<?= sanitize($parsedRel['bố']) ?>" placeholder="SĐT, tên...">
                        </div>
                        <div>
                            <label
                                style="font-size:12px;color:var(--text-muted);display:block;margin-bottom:4px;">Mẹ</label>
                            <input type="text" name="rel_me" class="form-input" style="font-size:13px;padding:6px 10px;"
                                value="<?= sanitize($parsedRel['mẹ']) ?>" placeholder="SĐT, tên...">
                        </div>
                        <div>
                            <label
                                style="font-size:12px;color:var(--text-muted);display:block;margin-bottom:4px;">Vợ/chồng</label>
                            <input type="text" name="rel_vochong" class="form-input"
                                style="font-size:13px;padding:6px 10px;" value="<?= sanitize($parsedRel['vợ/chồng']) ?>"
                                placeholder="SĐT, tên...">
                        </div>
                        <div>
                            <label
                                style="font-size:12px;color:var(--text-muted);display:block;margin-bottom:4px;">Anh/chị/em</label>
                            <input type="text" name="rel_anhchiem" class="form-input"
                                style="font-size:13px;padding:6px 10px;"
                                value="<?= sanitize($parsedRel['anh/chị/em']) ?>" placeholder="SĐT, tên...">
                        </div>
                        <div style="grid-column:1 / -1;">
                            <label style="font-size:12px;color:var(--text-muted);display:block;margin-bottom:4px;">Đồng
                                nghiệp</label>
                            <input type="text" name="rel_dongnghiep" class="form-input"
                                style="font-size:13px;padding:6px 10px;"
                                value="<?= sanitize($parsedRel['đồng nghiệp']) ?>" placeholder="SĐT, tên...">
                        </div>
                        <div style="grid-column: 1 / -1;">
                            <label style="font-size:12px;color:var(--text-muted);display:block;margin-bottom:4px;">Khác
                                (hoặc nội dung cũ ghi chú thêm)</label>
                            <textarea name="rel_khac" class="form-textarea"
                                style="font-size:13px;padding:6px 10px;min-height:50px;"
                                placeholder="Ghi chú thêm..."><?= sanitize($parsedRel['khac']) ?></textarea>
                        </div>
                    </div>
                </div>
        ...
        <form method="POST" id="transfer-form" onsubmit="return handleTransferSubmit()">
            ...
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <div
                        style="padding:10px 14px;background:var(--bg-elevated);border-radius:var(--radius-md);font-size:14px;">
                ...
                <div class="form-group">
                    ...
                    <textarea name="transfer_note" class="form-textarea" style="min-height:60px;"
                        placeholder="Lý do chuyển phòng..."></textarea>
                </div>
            </div>
            <div class="modal-footer">
        ...
        <form method="POST" id="wl-modal-form" onsubmit="return wlBeforeSubmit()">
            ...
            <div class="modal-body">
                    ...
                    <div id="wl-action-area">
                        <?php if (empty($currentRoomConfig)): ?>
                            <p style="color:var(--text-muted);font-size:13px;">⚠️ Phòng chưa cấu hình.
                                <a href="room_config.php?id=<?= $customer['room_id'] ?>"
                                    style="color:var(--accent-blue);">Cấu hình →</a>
                            </p>
                        <?php else: ?>
                            <div class="wl-modal-tags" id="wl-action-tags">
                                <?php foreach ($currentRoomConfig as $i => $ac): ?>
                                    <button type="button" class="wl-mtag" data-idx="<?= $i ?>"
                                        data-value="<?= sanitize($ac['action']) ?>" onclick="wlSelectAction(this, <?= $i ?>)">
                ...
                <div class="form-group" id="wl-custom-input-group" style="display:none;">
                    <label class="form-label" id="wl-custom-input-label"
                        style="color:var(--accent-orange);font-weight:600;"></label>
                ...
                <div class="form-group" id="wl-notes-group" style="display:none;">
                    ...
                    <textarea name="work_done" class="form-textarea" style="min-height:60px;"
